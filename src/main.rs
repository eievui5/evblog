...
impl IndexConfig {
    ...
    fn open(path: &Path) -> Self {
        ...
        match fs::read_to_string(path) {
            Ok(toml) => match toml.parse::<toml::Table>() {
                Ok(table) => {
                    if let Some(title) = table.get("title").map_or(None, |s| s.as_str()) {
                        config.title = title.to_string();
                    }
                    if let Some(tags) = table.get("tag").map_or(None, |s| s.as_array()) {
                        for tag in tags {
...
fn convert_document(cli: &Cli, infile: &Path, outfile: &Path) -> Metadata {
